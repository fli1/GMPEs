...
MBR17SZ.subCal <- function (ip, M, Zh, R, F_event, Vs30, 
                            F_DeltaC1, Delta_C1_input, F_FABA,
                           F_sigma=0, AddMedian = 0, epsilon=0) {
#   GMPE_CHILE_MBR(R,M,I,Zh,Vs30,F_FABA,epsilon)
...
#   period <- array(c(0.01, 0.04, 0.1, 0.2, 0.4, 1, 2, 1/0.33))
  # read coefficients
  file = sprintf("%s\\coeffs\\MBR17Coeffs.csv", getwd())
  ...
  if (M <= C1+Delta_C1) {
    ...
    f_mag_M0 = 0.80276784*(M-(C1+Delta_C10)) -0*(10-M)^2
    
  }  else {
    f_mag_M = theta5*(M-(C1+Delta_C1)) + theta13*(10-M)^2
    f_mag_M0 = -0.33486952*(M-(C1+Delta_C10)) -0*(10-M)^2
  }
  
  f_depth_Zh =  theta11 * (Zh-60)*F_event
  f_depth_Zh0 = 0.0056735 * (Zh-60)*F_event
  ...
  lnPGA1000  = 1.01494528*log(1000/865.1) -1.186*1.18* log(1000/865.1) 
  lnPGA1000  = lnPGA1000 + 5.87504394 + 0.80276784 * Delta_C10 
  lnPGA1000  = lnPGA1000 + (-1.75359772 -0.73080261*F_event + 0.13125248*(M-7.2))*log(R+C4*exp((M-6)*0.4)) 
  lnPGA1000  = lnPGA1000 -0.00039095*R + 4.53143081*F_event + f_mag_M0 + f_depth_Zh0 + f_FABA_R0
  ...
  lnY = lnY + (theta2+ theta14*F_event + theta3*(M-7.2))*log(R+C4*exp((M-6)*theta9)) + theta6*R 
...
MBR17SZ.Cal <- function (ip, M, Zh, R, Fevent, Vs30,  
                         F_DeltaC1, Delta_C1_input, F_FABA,
                         F_sigma, AddMedian = 0)
{
  ...
  if (length(which(period == ip)) == 0) {
    ...
    if (i_lo>=length(period))
    {
      i_lo <- length(period)
      T_lo <- period[i_lo]
      Sa_sigma <- MBR17SZ.subCal(T_lo,M,Zh, R, Fevent, Vs30, 
                                 F_DeltaC1, Delta_C1_input, F_FABA,
                                 F_sigma, AddMedian)
    ...
    else
    {
      T_lo <- period[i_lo]
      T_hi <- period[i_lo + 1]      
      Sa_sigma_hi <- MBR17SZ.subCal(T_hi,M,Zh, R, Fevent, Vs30,  
                                    F_DeltaC1, Delta_C1_input, F_FABA,F_sigma,AddMedian)
      Sa_sigma_lo <- MBR17SZ.subCal(T_lo,M,Zh, R, Fevent, Vs30, 
                                    F_DeltaC1, Delta_C1_input, F_FABA,F_sigma,AddMedian)
  ...
  if (length(which(period == ip)) > 0) {
    Sa_sigma <- MBR17SZ.subCal(ip, M, Zh, R, Fevent, Vs30, 
                               F_DeltaC1, Delta_C1_input, F_FABA,F_sigma,AddMedian)
  }
  return (Sa_sigma)
}
    ...
    for (j in 1:m)
    {
      for (k in 1:n)
      {
        for (t in 1:le) {
          ...
          results <- MBR17SZ.Cal(list.p[t], list.mag[j], list.zh[hh], 
