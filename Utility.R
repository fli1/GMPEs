...
GetSubdata <- function(output.sa, output.sigma, list.p,
  ...
  if(flag.scenario=="distance") {  ## by default, read the results for the 1st magnitude and depth
    ...
    temp = cbind(rep(output.sigma[ind.p,1],length(out)),
                 rep(output.sigma[ind.p,2],length(out)),
                 rep(output.sigma[ind.p,3],length(out)))
    output.sigma2 = temp ##[rep(1, each=length(out)),]
    
    out = cbind(out, output.sigma2)
  }
  if(flag.scenario=="magnitude") {  ## by default, read the results for the 1st magnitude and depth
    
    ###!!!!! some sigma is magnitude-dependent; The code needs to be updated to include this case
    out = output.sa[1,,ind.p,1]
    temp = cbind(rep(output.sigma[ind.p,1],length(out)),
                 rep(output.sigma[ind.p,2],length(out)),
                 rep(output.sigma[ind.p,3],length(out)))
...
plot.comp <- function (figname, datain, data.recorded, 
  ...
  lwd <- c(rep(c(2,2,3,3),n))
  lty <- c(rep(1:2,n)) #c(rep(c(1,2),n))
  col <- pal
  ...
  {
    lines(datain[,1], datain[,(j-1)*4+2], col = col[j], lwd = lwd[j], lty=lty[j])
  }
...
plot.comp84th <- function (figname, datain, data.recorded, 
                       list.GMPEs, flag.scenario, flag.period, 
                       flag.source,vs30,
                       list.mag, list.dist, list.zh, ymax){
  ...
  mtext(xlabel, side = 1, line = 2.5, cex=0.95)
  mtext("84th-percentile Spectral Acceleration (g)", side = 2, line = 2.5, cex=0.95)
  mtext(sprintf("%s", figtitle),
        side = 3, line = 1, cex=0.95)
  ...
  for(j in 1:length(list.GMPEs))  {
    lines(datain[,1], exp(log(datain[,(j-1)*4+2])+datain[,(j-1)*4+3]), 
